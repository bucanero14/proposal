...
$(document).ready(function() {
	...
	$(".countdown").TimeCircles({
		...
		time: {
			Days: { color: '#4DD0E2' },
			Hours: { color: '#4DD0E2' },
			Minutes: { color: '#4DD0E2' },
			Seconds: { color: '#4DD0E2' }
		},
	...
	var hoy = new Date();
	var ye = new Intl.DateTimeFormat('es', {year: 'numeric'}).format(hoy);
	var mo = new Intl.DateTimeFormat('es', {month: 'long'}).format(hoy);
	var da = new Intl.DateTimeFormat('es', {day: '2-digit'}).format(hoy);
	$('#fecha-compromiso').html(mo.charAt(0).toUpperCase() + mo.slice(1) + ' <span>' + da + '</span> ' + ye);
});